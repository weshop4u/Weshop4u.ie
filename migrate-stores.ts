...
async function migrateStores() {
  try {
    ...
    const [stores] = await mysqlConnection.execute("SELECT * FROM stores") as any;
    console.log(`Found ${(stores as any[]).length} stores in Manus`);
    ...
    console.log(`✓ Migrated ${(stores as any[]).length} stores to Railway`);
