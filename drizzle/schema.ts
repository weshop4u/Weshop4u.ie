...
export const stores = pgTable(
  "stores",
  {
    ...
    autoPrintThreshold: serial("auto_print_threshold").default(5), // Auto-print orders with this many items or more
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (table) => ({
...
export const productCategories = pgTable("product_categories", {
  ...
  sortOrder: serial("sort_order").default(0), // For custom ordering in the UI
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const drivers = pgTable(
  "drivers",
  {
    ...
    totalReturns: serial("total_returns").default(0),
...
export const orders = pgTable(
  "orders",
  {
    ...
    customerId: serial("customer_id"), // Nullable for guest orders
    storeId: serial("store_id").notNull(),
    ...
    discountCodeId: serial("discount_code_id"),
    ...
    batchSequence: serial("batch_sequence"), // Delivery order within the batch (1 = first delivery)
...
export const driverQueue = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    driverId: serial("driver_id").notNull(), // This is the user ID of the driver
...
export const orderOffers = pgTable(
  "order_offers",
  {
    id: serial("id").primaryKey(),
    orderId: serial("order_id").notNull(),
    driverId: serial("driver_id").notNull(), // User ID of the driver being offered
...
export const driverRatings = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    orderId: serial("order_id").notNull().unique(), // One rating per order
    driverId: serial("driver_id").notNull(), // User ID of the driver
    customerId: serial("customer_id").notNull(), // User ID of the customer
...
export const modifierGroups = pgTable(
  "modifier_groups",
  {
    ...
    minSelections: serial("min_selections").default(0), // For multi: minimum picks
    maxSelections: serial("max_selections").default(0), // For multi: max picks (0 = unlimited)
...
export const multiBuyDeals = pgTable(
  "multi_buy_deals",
  {
    id: serial("id").primaryKey(),
    productId: serial("product_id").notNull(),
    quantity: serial("quantity").notNull(), // e.g. 2
...
export const orderItemModifiers = pgTable(
  "order_item_modifiers",
  {
    ...
    modifierId: serial("modifier_id"), // Reference to original modifier (nullable if deleted)
...
export const modifierTemplates = pgTable("modifier_templates", {
  ...
  minSelections: serial("min_selections").default(0),
  maxSelections: serial("max_selections").default(0),
  allowOptionQuantity: boolean("allow_option_quantity").default(false), // When true, each option gets +/− stepper instead of checkbox
  maxOptionQuantity: serial("max_option_quantity").default(6), // Max quantity per individual option (e.g. max 6 sausages)
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const discountCodes = pgTable(
  "discount_codes",
  {
    ...
    // Usage limits
    maxUsesTotal: serial("max_uses_total"), // null = unlimited
    maxUsesPerCustomer: serial("max_uses_per_customer").default(1), // default 1 use per customer
    currentUsesTotal: serial("current_uses_total").default(0),
    // Validity period
...
export const discountUsage = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    discountCodeId: serial("discount_code_id").notNull(),
...
export const promotionalBanners = pgTable("promotional_banners", {
  ...
  sortPosition: serial("sort_position").default(0).notNull(),
...
export const driverShifts = pgTable(
  "driver_shifts",
  {
    ...
    // Settlement tracking
    totalJobs: serial("total_jobs").default(0),
    ...
    settledBy: serial("settled_by"), // Admin user ID who marked it settled
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => ({
